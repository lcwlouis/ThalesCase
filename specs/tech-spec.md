...
- HEX module: converts `ArrayBuffer`/`Uint8Array` to lowercase HEX and, if optional import is implemented, validates/parses pasted HEX.
- ECDSA module: generates, imports, exports, signs, and verifies P-256 keys/signatures through Web Crypto.
- Optional reviewer fixture: committed non-secret sample values for manual copy/paste verification.
...
- If optional reviewer import controls are implemented, place them after or below the generated-key flow so the required assignment path remains the primary path.
...
- Signature display format: raw ECDSA `r || s`, 64 bytes, 128 lowercase HEX characters.
...
## Key Import and Optional Reviewer Fixture

The core assignment requires key generation, signing, verification, and HEX display. Copy/paste key import is an optional stretch feature that may be added for reviewer-provided key material.

If copy/paste import is implemented:
...
The repository includes `app/settings.example.json` as an optional committed test fixture. It is not a secret store and is not required for the core app flow. It exists so a reviewer can request testing against a specific key, message, or signature.
...
- Invalid key import, if optional import is implemented: show a clear message that the key must be P-256 in the documented DER HEX format.
- Missing private key on sign: ask the user to generate a keypair or, if optional import is implemented, paste a private key.
...
- Pasted private keys, if optional import is implemented, are local test fixtures and not confidential secret material.
